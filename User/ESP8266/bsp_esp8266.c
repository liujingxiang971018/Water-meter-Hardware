...
static void ESP8266_USART_Config ( void )
{
	...
	GPIO_InitStructure.GPIO_Pin =  macESP8266_USART_TX_PIN;//发送引脚推挽复用输出
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(macESP8266_USART_TX_PORT, &GPIO_InitStructure);  
  
	/* Configure USART Rx as input floating */
	GPIO_InitStructure.GPIO_Pin = macESP8266_USART_RX_PIN;//接收引脚浮空输入
...
bool ESP8266_JoinAP ( char * pSSID, char * pPassWord )
{
	...
	sprintf ( cCmd, "AT+CWJAP=\"%s\",\"%s\"", pSSID, pPassWord );//sprintf将数据以ascii码打印到缓冲中
	
	return ESP8266_Cmd ( cCmd, "OK", NULL, 4000 );
...
bool ESP8266_Enable_MultipleId ( FunctionalState enumEnUnvarnishTx )
{
	...
	sprintf ( cStr, "AT+CIPMUX=%d", ( enumEnUnvarnishTx ? 1 : 0 ) );//1表示多连接，0表示单连接
...
char *ESP8266_MACGET(void)
{
	...
	//ipmac=strEsp8266_Fram_Record.Data_RX_BUF;
...
//		printf( "%c\r\n",*mac++);//57-73
//	}
	for(count=56;count<73;count++)
	{
		mac[count-56]=strEsp8266_Fram_Record.Data_RX_BUF[count];
	}
	mac[count]='\0';//空格结束符
	//mac_ptr=mac;
	printf( "%s\r\n",mac);
